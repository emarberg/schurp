...
def flagged_tab(k: int, mu: tuple) -> list:
...
def rpp(k, mu):
    # TODO
    ans = []
    flagged = flagged_tab(k, mu)
    for t in flagged:
        f = Tableau({ij: v - ij[0] + 1 for (ij, v) in t.mapping.items()})
        #
        # fmapping = {}
        # for ij in t.mapping:
        #     v = t.mapping[ij]
        #     fmapping[ij] = v - ij[0] + 1
        # f = Tableau(fmapping)
        #
        # f = Tableau()
        # for i, j in t:
...
def sigma_fpf(k, mu):
    # converts mu=(mu1, mu2, ...) to (mu1+k, ..., mu1+2, mu1+1, mu1, mu2, ...)
    maxpart = mu[0] if mu else 0
